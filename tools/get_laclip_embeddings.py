...
def process_annotations(file_path, output_path, model_path):
    # Load the JSON data
    ...
    # Load the pre-trained LaCLIP model
    model, _ = clip.load("ViT-B/16", device=device)  # Adjust model type if necessary
    checkpoint = torch.load(model_path, map_location=device)
    model.load_state_dict(checkpoint["state_dict"], strict=False)  # Load checkpoint weights
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Get LaCLIP embeddings for annotations')
    ...
    parser.add_argument('--model_path', type=str, required=True,
                        help='Path to the LaCLIP model checkpoint')

    args = parser.parse_args()

    # Call the function with the file path
    process_annotations(args.file_path, args.output_path, args.model_path)
